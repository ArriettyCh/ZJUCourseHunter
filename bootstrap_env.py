...
REQUIRED_MODULES = ("requests", "loguru", "playwright")
...
def _install_missing_python_deps(requirements_path: str, missing_modules: list[str]):
    ...
    missing_set = {m.lower() for m in missing_modules}
    selected = [req for req in reqs if _extract_package_name(req) in missing_set]

    # requirements 里找不到时，直接按模块名安装
